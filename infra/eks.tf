...
resource "null_resource" "helm_chart" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      aws eks update-cluster-config --region ${data.aws_region.this.region} --name ${one(aws_eks_cluster.this.*.name)} --resources-vpc-config publicAccessCidrs="$(curl -s https://checkip.amazonaws.com)/32" && \
      aws eks wait cluster-active --region ${data.aws_region.this.region} --name ${one(aws_eks_cluster.this.*.name)} && \
      helm repo add jupyterhub https://hub.jupyter.org/helm-chart/ && helm repo update && \
      helm upgrade --cleanup-on-fail \
        --install jupyterhub jupyterhub/jupyterhub \
        --kube-apiserver="$(aws eks describe-cluster --region ${data.aws_region.this.region} --name ${one(aws_eks_cluster.this.*.name)} --query "cluster.endpoint" --output text)" \
